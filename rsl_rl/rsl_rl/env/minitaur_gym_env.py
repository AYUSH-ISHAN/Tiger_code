...
class MinitaurGymEnv(gym.Env):
  ...
  def reset(self, initial_motor_angles=None, reset_duration=1.0):
    ...
    self._hurestics = deque(maxlen=5)

    for i in range(5):
      self._hurestics.append([np.zeros(28, dtype=np.float32), np.zeros(8 ,dtype=np.float32())\
                              , np.zeros(1, dtype=np.float32())])
      
    hurestics = self.call_hurestics()
      
    return self._get_observation(), hurestics
  ...
  def step(self, action):
    ...
    self._hurestics.append([np.array(self._get_observation(), dtype=np.float32()), \
              np.array(action, dtype=np.float32()), np.array(reward, dtype=np.float32()).reshape(1)])
    return np.array(self._get_observation(), dtype=np.float32), np.array(reward, dtype=np.float32), np.array(done), {}

  def call_hurestics(self):
    # returns hurestics -- [obs, action, reward]  like this 5 list.
    obs, act, reward = [], [], []
    for i in range(5):
        obs.append(self._hurestics[i][0])
        act.append(self._hurestics[i][1])
        reward.append(self._hurestics[i][2])
