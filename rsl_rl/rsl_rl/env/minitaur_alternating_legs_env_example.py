...
from envs.minitaur.envs import minitaur_alternating_legs_env
from envs.minitaur.envs import minitaur_gym_env
from envs.minitaur.envs.env_randomizers import minitaur_alternating_legs_env_randomizer as randomizer_lib
...
def hand_tuned_balance_example(log_path=None):
  ...
  for i in range(episodes):
    ...
    for _ in range(steps):
      ...
      if done:
        break
    tf.compat.v1.logging.info("reward {}: {}".format(i, sum_reward))
    avg_reward += sum_reward
  tf.compat.v1.logging.info("avg_reward: {}\n\n\n".format(avg_reward / episodes))
